...
class InstaBot:
    ...
    def populate_user_blacklist(self):
        for user in self.user_blacklist:
            ...
            log_string = "Users nao desejados %s adicionados com ID: %s" % (user, id_user)
            self.write_log(log_string)
            time.sleep(5 * random.random())

        log_string = "blacklist carregada"
        self.write_log(log_string)

    def login(self):
        log_string = 'A aldrabar o insta como %s...\n' % (self.user_login)
        self.write_log(log_string)
        ...
        if login.status_code == 200:
            ...
            if finder != -1:
                ...
                log_string = '%s login com sucesso!' % (self.user_login)
                self.write_log(log_string)
            else:
                self.login_status = False
                self.write_log('Erro de login!')
        else:
            self.write_log('Erro de login!')
    ...
    def get_media_id_by_tag(self, tag):
        ...
        if (self.login_status):
            log_string = "A procurar por tag: %s" % (tag)
            self.write_log(log_string)
    ...
    def like_all_exist_media(self, media_size=-1, delay=True):
        ...
        if (self.login_status):
            if self.media_by_tag != 0:
                        ...
                        if ((l_c <= self.media_max_like and l_c >= self.media_min_like)
                            ...
                            for blacklisted_user_name, blacklisted_user_id in self.user_blacklist.items():
                                if (self.media_by_tag[i]['owner']['id'] == blacklisted_user_id):
                                    self.write_log("Nao gostar de media vinda de user da blacklist: " + blacklisted_user_name)
                                    return False
                            if (self.media_by_tag[i]['owner']['id'] == self.user_id):
                                self.write_log("Calma... ;)")
                                return False
                            ...
                            try:
                                ...
                                if tags.intersection(tag_blacklist):
                                        ...
                                        self.write_log("Nao gostar de media com as tag(s): " + matching_tags)
                                        return False
                            except:
                                self.write_log("Abortar like")
                                return False

                            log_string = "A colocar like: %s" % \
                            ...
                            if like != 0:
                                if like.status_code == 200:
                                    ...
                                    self.error_400 = 0
                                    self.like_counter += 1
                                    log_string = "Gostado: %s. Like #%i." % \
                                                 (self.media_by_tag[i]['id'],
                                                  self.like_counter)
                                    self.write_log(log_string)
                                elif like.status_code == 400:
                                    log_string = "Nao por like: %i" \
                                                 % (like.status_code)
                                    self.write_log(log_string)
                                    # Erro, se repetir pode ser ban!
                ...
                self.write_log("Sem media para por like!")
        ...
        if (self.login_status):
            ...
            try:
                ...
                if comment.status_code == 200:
                    ...
                    log_string = 'Escrever: "%s". #%i.' % (comment_text, self.comments_counter)
                    self.write_log(log_string)
                return comment
            except:
                self.write_log("Nop no comentario!")
        return False
        ...
        if (self.login_status):
            ...
            try:
                ...
                if follow.status_code == 200:
                    ...
                    log_string = "A seguir: %s #%i." % (user_id, self.follow_counter)
                    self.write_log(log_string)
                return follow
            except:
                self.write_log("Nop no follow!")
        return False
        ...
        if (self.login_status):
            ...
            try:
                ...
                if unfollow.status_code == 200:
                    ...
                    log_string = "A retirar follow: %s #%i." % (user_id, self.unfollow_counter)
                    self.write_log(log_string)
                return unfollow
            except:
                self.write_log("Nop no unfollow!")
        return False
    ...
    def unfollow_on_cleanup(self, user_id):
        ...
        if (self.login_status):
            ...
            try:
                ...
                if unfollow.status_code == 200:
                    self.unfollow_counter += 1
                    log_string = "Retirado follow: %s #%i of %i." % (user_id, self.unfollow_counter, self.follow_counter)
                    self.write_log(log_string)
                else:
                    log_string = "CALMA CARALHO, QUERES LEVAR BAN SUA LIKE-WHORE"
                    self.write_log(log_string)
                    time.sleep(300)
                    unfollow = self.s.post(url_unfollow)
                    if unfollow.status_code == 200:
                        ...
                        log_string = "Tá quase... :( parar mais 5 minutos"
                        self.write_log(log_string)
                        time.sleep(300)
                    return False
                return unfollow
            except:
                log_string = "Liga para a NOS..."
                self.write_log(log_string)
        return False
    ...
    def new_auto_mod_follow(self):
        if time.time() > self.next_iteration["Follow"] and \
            ...
            if self.media_by_tag[0]["owner"]["id"] == self.user_id:
                ...
                return
            log_string = "A tentar seguir (id, testar geoloc na proxima build): %s" % (self.media_by_tag[0]["owner"]["id"])
            self.write_log(log_string)
            ...
            if (self.bot_mode == 0) :
                for f in self.bot_follow_list:
                    if time.time() > (f[1] + self.follow_time):
                        log_string = "A tentar dar unfollow #%i: "
                        self.write_log(log_string)
                        self.auto_unfollow()
                        self.bot_follow_list.remove(f)
    ...
    def new_auto_mod_comments(self):
        if time.time() > self.next_iteration["Comments"] and self.comments_per_day != 0 \
            ...
            log_string = "A tentar comentar: %s" % (self.media_by_tag[0]['id'])
            self.write_log(log_string)
    ...
    def check_exisiting_comment(self, media_code):
        ...
        for d in comment_list:
            if d['user']['id'] == self.user_id:
                self.write_log("Media ja foi comentada ;)")
                # Del media to don't loop on it
                del self.media_by_tag[0]
                return True
        return False
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    while i<1:
                        ...
                        log_string = "Seguidores : %i" % (follower)
                        self.write_log(log_string)
                        log_string = "A seguir : %s" % (follows)
                        self.write_log(log_string)
                        ...
                        if follow_viewer or has_requested_viewer:
                            ...
                            print("   >>>esta a seguir-te")
                        else:
                            self.is_follower = False
                            print('   >>>nao foste seguido')

                        if followed_by_viewer or requested_by_viewer:
                            self.is_following = True
                            print('   >>>estas a seguir esta conta')

                        else:
                            self.is_following = False
                            print('   >>>nao estas a seguir esta conta')
                        i+=1

                except:
                    media_on_feed = []
                    self.write_log("Nop no get_info!")
                    time.sleep(20)
                    return 0
            else:
                return 0
    ...
    def get_media_id_recent_feed (self):
        if (self.login_status):
            now_time = datetime.datetime.now()
            log_string = "%s : Obter media do feed recente"%(self.user_login)
            self.write_log(log_string)
            if self.login_status == 1:
                ...
                try:
                    ...
                    self.media_on_feed = []
                    self.write_log("Nop no get_media!")
                    time.sleep(20)
                    return 0
            else:
                return 0
